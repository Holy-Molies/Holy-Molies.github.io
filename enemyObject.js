...
function EnemyObject(enemyType) {
    ...
    this.animateEnemy = function () {
        ...
        enemyContext.clearRect(260, 150, 188, 163);
        //Sets starting upper left corner of image on the canvas
        enemyContext.drawImage(animationFrames[frame], 260, 150);
        //frame = (frame + 1) % frames.length;//run for loop to fully animate
        frame = (frame + 1);
        //alert("HERE!");
        if (frame == animationFrames.length) {
            clearInterval(animated);//Need to define if we need to create a variable for burgerAttack or use this.attack?
            frame = 0;
            animated = null;
        }
    ...
    this.attackEnemy = function () {
        if (!animated) {
            animated = setInterval(this.animateEnemy, frameRate);
        }
    }
...
function createEnemy()
{
    ...
    var randomNum = Math.floor(Math.random() * enemyType.length); //0,1,2
...
function startEnemyAttack() {
    enemyAttack = setInterval(function () {
        enemies[0].attack();
    }, attackRate); //have the enemy attack the character periodically
    //enemies[0].attack();
